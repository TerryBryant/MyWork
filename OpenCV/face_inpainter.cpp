...
{
	ErrorFaceInpainterOriImageFileLoad = 2,    //µ¼ÈëÔ­Ê¼Í¼Ïñ
	ErrorFaceInpainterMaskImageFileLoad,       //µ¼ÈëMask
	ErrorFaceInpainterKeyPointsLoad,               //µ¼ÈëËùÓÐÌØÕ÷µã
	ErrorFaceInpainterMatToBuffer,               //Ëã·¨×îºó£¬½«ÐÞ¸´ÍêµÄMat¸ñÊ½Í¼Ïñ×ªÎªbuffer
};

// µ¥ÔªÐÞ¸´ÇøÓò³ß´çµÄÀàÐÍÑ¡Ôñ
enum class PatchSizeType
{
	AutoPatchSize = 0,   //³ÌÐò×Ô¶¯ÅÐ¶Ï³ß´ç
	BigPatchSize,
	SmallPatchSize
};
...
IplImage* CdvImageInterfaceToIplImage(CdvImageInterface* pSrcImg)
{
	...
	if ((tmpSrcWidth == pSrcImg->GetRoi().nWidth && tmpSrcHeight == pSrcImg->GetRoi().nHeight))
	{
		...
		if (NULL != tmpDstImg)
		{
			...
			{
				//Èç¹ûÊÇ»Ò¶ÈÍ¼»òÕßÍ¼ÏñÊý¾ÝÍ¨µÀË³ÐòÒ»ÖÂ
				memcpy(tmpDstImgData, tmpSrcImgData, tmpSrcHeight * tmpSrcWidthstep * sizeof(char));
			}
			else
			{
				//Èç¹ûCdvImageInterfaceµÄÍ¨µÀË³ÐòÓëIplImageµÄ²»Ò»ÖÂ
