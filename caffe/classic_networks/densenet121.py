...
def DenseNet121Body(net, from_layer, use_pool5=True, use_dilation_conv5=False, **bn_param):
    conv_prefix = ''
    conv_postfix = ''
    bn_prefix = ''
    bn_postfix = '/bn'
    ...
    net.pool4 = L.Pooling(net[net.keys()[-1]], pool=P.Pooling.AVE, kernel_size=2, stride=2, pad=0)
...
net.data = L.Data(batch_size=1, backend=P.Data.LMDB, source='/mnt/data/ys/lmdb',
                             transform_param=dict(scale=1./255, crop_size=224)
                             )
DenseNet121Body(net, from_layer='data', use_pool5=True, use_dilation_conv5=False)
...
net.fc6 = L.Convolution(net[net.keys()[-1]], num_output=1000, kernel_size=1,
                        pad=0, stride=1, **kwargs)
net.prob = L.Softmax(net[net.keys()[-1]])

with open('train_densenet121.prototxt', 'w') as f:
    print('name: "{}_train"'.format('densenet121'), file=f)
